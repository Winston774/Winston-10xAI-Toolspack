# Index Studio for IndexTTS 2.5

這是以 [IndexTTS 2.5](https://github.com/index-tts/index-tts) 為核心的 Windows 本機語音實驗工作台，加入臺灣正體中文流程、單檔 HTML 介面、五語言合成、情緒／語速／發音控制、可重現參數與本機生成紀錄。
...
1. 安裝 [Git](https://git-scm.com/downloads) 與 [uv](https://docs.astral.sh/uv/getting-started/installation/)。
2. 下載或 clone 本 Repository。
3. Windows 雙擊 `start_htmlui.bat`，或執行 `./start_htmlui.ps1`。
4. 等待首次環境與模型下載完成，再開啟 `http://127.0.0.1:7861`。

首次安裝會建立約 8 GB 的 Python 環境，IndexTTS 2.5 模型與輔助模型另需數 GB。模型權重、私人聲音、生成 WAV、JSON 紀錄與本機環境都不包含在 Repository 內。

## 學員入口

- [繁體中文操作與架構](docs/HTML_UI_ZH.md)
- [能力實驗流程](docs/CAPABILITY_EXPERIMENTS_ZH.md)
- [學員快速導讀](docs/STUDENT_GUIDE_ZH.md)
- [隱私、聲音權利與安全](docs/PRIVACY_AND_VOICE_RIGHTS_ZH.md)
- [驗證與驗收](docs/VERIFICATION_ZH.md)
- [上游來源與衍生作品資訊](NOTICE.md)

## 授權邊界

本專案沿用上游 `bilibili Model Use License Agreement`，完整條款見 [LICENSE](LICENSE) 與 [LICENSE_ZH.txt](LICENSE_ZH.txt)。此授權包含額外的散布、下游使用、商業規模、高風險用途與模型改進限制；請在下載、執行、修改或散布前完整閱讀。這個 Repository 未宣告 MIT 授權。
...
### Standalone HTML Workbench

This fork also includes a dependency-free single-file frontend for capability experiments. On Windows, run `start_htmlui.bat`, or use:

```bash
uv run --extra htmlui python htmlui_server.py
```

The local workbench opens at `http://127.0.0.1:7861`. It covers a Taiwan Traditional Chinese mode with internal OpenCC conversion, multilingual synthesis, independent emotion controls, duration factor, pronunciation annotations, reproducible sampling settings, WAV downloads, and JSON run metadata. The browser and metadata retain the original Traditional Chinese text while only the internal model request is converted to Simplified Chinese. See [the Traditional Chinese HTML UI guide](docs/HTML_UI_ZH.md) and [capability experiment protocol](docs/CAPABILITY_EXPERIMENTS_ZH.md) for the workflow and API contract.
