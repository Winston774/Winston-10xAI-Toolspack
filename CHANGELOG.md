...
## 2026-W39 v0.1.0 Preview — 2026-09-24

- 新增 Jev 決策遊樂場：本機啟發式預演、訪客金鑰即時模式、三層決策、提案與否決、紀錄匯出。
- 採用來源獨立 GPTsite 最新 Worker 修正，排除個人部署設定、郵件、金鑰、研究素材與私人輸出。
- 加入 Node.js 獨立建置、Windows 啟動入口、11 項離線測試、教材、資料流、限制與疑難排解。
- 本次沒有使用有效金鑰呼叫 Jev；維持 Preview，實際模型表現、費用與端到端 live 驗收留待使用者帳戶測試。

## 2026-W38 v0.1.1 Preview — 2026-09-17

- 依使用者更正的 video-understanding2.rar 更新同一個 W38，11 個功能／設定檔與原始文字一致；SKILL 僅移除授權檔引用，驗證紀錄依本次結果重寫。
- 重新編寫學員入口、講義、限制、疑難排解、分享、來源與驗證說明，統一新版本下載入口。
- 移除前版額外加入的 Hypit 研究文件、安全附加流程與合成示例；依使用者指示刪除 W38 LICENSE／NOTICE，授權欄位設為未指定，ZIP 不附根目錄 LICENSE。
